...
<div class="container mx-auto p-4 md:p-8 max-w-2xl">
    ...
    <div class="mb-4">
        <c:if test="${not empty tipoEntrada.idFestival}">
            <a href="${pageContext.request.contextPath}/api/promotor/festivales/ver/${tipoEntrada.idFestival}" class="text-indigo-600 hover:text-indigo-800 text-sm">&larr; Volver a Detalles del Festival</a>
        </c:if>
    </div>
    ...
    <form action="${pageContext.request.contextPath}/api/promotor/tiposEntrada/${tipoEntrada.idEntrada}/actualizar" method="post" class="bg-white p-6 md:p-8 rounded-lg shadow-md space-y-4">
        ...
        <div>
            ...
            <input type="text" id="tipo" name="tipo" value="${tipoEntrada.tipo}" required maxlength="50"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" id="precio" name="precio" value="${tipoEntrada.precio}" required min="0" step="0.01"
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50 sm:text-sm">
            </div>
            <div>
                ...
                <input type="number" id="stock" name="stock" value="${tipoEntrada.stock}" required min="0" step="1"
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50 sm:text-sm">
            </div>
        </div>
